...
void IRGenerator::generateExternalFunction(ContractDefinition const& _contract, FunctionType const& _functionType)
{
	string functionName = IRNames::externalFunctionPart(_functionType.declaration());
	m_context.functionCollector().createFunction(functionName, [&](vector<string>&, vector<string>&) -> string {
		Whiskers t(R"X(
			<callValueCheck>
...
string IRGenerator::dispatchRoutine(ContractDefinition const& _contract)
{
	...
	for (auto const& function: _contract.interfaceFunctions())
	{
		...
		templ["externalFunction"] = IRNames::externalFunctionPart(type->declaration());
		generateExternalFunction(_contract, *type);
	}
	t("cases", functions);
