...
Json formatFatalError(string const& _type, string const& _message)
{
	...
	output["errors"].emplace_back(formatError(Error::Severity::Error, _type, "general", _message));
	return output;
}
...
std::optional<Json> checkOptimizerDetailSteps(Json const& _details, std::string const& _name, string& _setting)
{
	if (_details.contains(_name))
	{
		if (_details[_name].is_string())
		{
			...
			_setting = _details[_name].get<string>();
		}
		else
...
//	if (!_input.contains("settings"))
//		_input["settings"] = Json::object();
	...
	for (auto const& remapping: settings["remappings"])
	{
		...
		if (auto r = ImportRemapper::parseRemapping(remapping.get<string>()))
			...
			return formatFatalError("JSONError", "Invalid remapping: \"" + remapping.get<string>() + "\"");
	}
...
//		settings["libraries"] = Json::object();

	Json jsonLibraries = settings["libraries"];
...
//		settings["metadata"] = Json::object();
	Json metadataSettings = settings["metadata"];
...
Json StandardCompiler::compileYul(InputsAndSettings _inputsAndSettings)
{
	...
	output["errors"] = Json::array();
	output["errors"].emplace_back(formatError(Error::Severity::Warning, "Warning", "general", "Yul is still experimental. Please use the output with care."));
