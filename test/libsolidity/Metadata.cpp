...
optional<string> compileAndCheckLicenseMetadata(string const& _contractName, char const* _sourceCode)
{
	...
	BOOST_REQUIRE(metadata["sources"].count("A.sol"));

	if (metadata["sources"]["A.sol"].count("license"))
	{
...
BOOST_AUTO_TEST_CASE(metadata_relevant_sources)
{
	...
	BOOST_CHECK_EQUAL(metadata["sources"].size(), 1);
	BOOST_CHECK(metadata["sources"].count("A"));
}
...
BOOST_AUTO_TEST_CASE(metadata_relevant_sources_imports)
{
	...
	BOOST_CHECK(metadata["sources"].count("A"));
	BOOST_CHECK(metadata["sources"].count("B"));
	BOOST_CHECK(metadata["sources"].count("C"));
}
...
BOOST_AUTO_TEST_CASE(metadata_useLiteralContent)
{
	...
	auto check = [](char const* _src, bool _literal)
	{
		...
		BOOST_CHECK(solidity::test::isValidMetadata(metadata));
		BOOST_CHECK(metadata.count("settings"));
		BOOST_CHECK(metadata["settings"].count("metadata"));
		BOOST_CHECK(metadata["settings"]["metadata"].count("bytecodeHash"));
		if (_literal)
		{
			BOOST_CHECK(metadata["settings"]["metadata"].count("useLiteralContent"));
...
BOOST_AUTO_TEST_CASE(metadata_viair)
{
	...
	auto check = [](char const* _src, bool _viaIR)
	{
		...
		if (_viaIR)
		{
			BOOST_CHECK(metadata["settings"].count("viaIR"));
