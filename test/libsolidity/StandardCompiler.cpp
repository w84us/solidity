...
bool containsAtMostWarnings(Json const& _compilerResult)
{
	...
	for (auto const& error: _compilerResult["errors"])
	{
		...
		if (langutil::Error::isError(str2Severity(error["severity"].get<string>())))
			return false;
	}
...
Json getContractResult(Json const& _compilerResult, string const& _file, string const& _name)
{
	if (
...
void expectLinkReferences(Json const& _contractResult, map<string, set<string>> const& _expectedLinkReferences)
{
...
BOOST_AUTO_TEST_CASE(basic_compilation)
{
	...
	BOOST_CHECK_EQUAL(
		solidity::test::bytecodeSansMetadata(contract["evm"]["bytecode"]["object"].get<string>()),
	...
	BOOST_CHECK(contract["evm"]["assembly"].get<string>().find(
	...
	BOOST_CHECK_EQUAL(
		u256(contract["evm"]["gasEstimates"]["creation"]["codeDepositCost"].get<string>()) +
		u256(contract["evm"]["gasEstimates"]["creation"]["executionCost"].get<string>()),
		u256(contract["evm"]["gasEstimates"]["creation"]["totalCost"].get<string>())
	);
	...
	BOOST_CHECK(solidity::test::isValidMetadata(contract["metadata"].get<string>()));
...
BOOST_AUTO_TEST_CASE(compilation_error)
{
	...
	for (auto const& error: result["errors"])
	{
		BOOST_REQUIRE(error.is_object());
		BOOST_REQUIRE(error["message"].is_string());
		if (error["message"].get<string>().find("pre-release compiler") == string::npos)
		{
			BOOST_CHECK_EQUAL(
...
BOOST_AUTO_TEST_CASE(evm_version)
{
	...
	BOOST_CHECK(result["contracts"]["fileA"]["A"]["metadata"].get<string>().find("\"evmVersion\":\"homestead\"") != string::npos);
	result = compile(inputForVersion("\"evmVersion\": \"tangerineWhistle\","));
	BOOST_CHECK(result["contracts"]["fileA"]["A"]["metadata"].get<string>().find("\"evmVersion\":\"tangerineWhistle\"") != string::npos);
	result = compile(inputForVersion("\"evmVersion\": \"spuriousDragon\","));
	BOOST_CHECK(result["contracts"]["fileA"]["A"]["metadata"].get<string>().find("\"evmVersion\":\"spuriousDragon\"") != string::npos);
	result = compile(inputForVersion("\"evmVersion\": \"byzantium\","));
	BOOST_CHECK(result["contracts"]["fileA"]["A"]["metadata"].get<string>().find("\"evmVersion\":\"byzantium\"") != string::npos);
	result = compile(inputForVersion("\"evmVersion\": \"constantinople\","));
	BOOST_CHECK(result["contracts"]["fileA"]["A"]["metadata"].get<string>().find("\"evmVersion\":\"constantinople\"") != string::npos);
	result = compile(inputForVersion("\"evmVersion\": \"petersburg\","));
	BOOST_CHECK(result["contracts"]["fileA"]["A"]["metadata"].get<string>().find("\"evmVersion\":\"petersburg\"") != string::npos);
	result = compile(inputForVersion("\"evmVersion\": \"istanbul\","));
	BOOST_CHECK(result["contracts"]["fileA"]["A"]["metadata"].get<string>().find("\"evmVersion\":\"istanbul\"") != string::npos);
	result = compile(inputForVersion("\"evmVersion\": \"berlin\","));
	BOOST_CHECK(result["contracts"]["fileA"]["A"]["metadata"].get<string>().find("\"evmVersion\":\"berlin\"") != string::npos);
	result = compile(inputForVersion("\"evmVersion\": \"london\","));
	BOOST_CHECK(result["contracts"]["fileA"]["A"]["metadata"].get<string>().find("\"evmVersion\":\"london\"") != string::npos);
	// test default
	result = compile(inputForVersion(""));
	BOOST_CHECK(result["contracts"]["fileA"]["A"]["metadata"].get<string>().find("\"evmVersion\":\"london\"") != string::npos);
	// test invalid
	result = compile(inputForVersion("\"evmVersion\": \"invalid\","));
	BOOST_CHECK(result["errors"][0]["message"].get<string>() == "Invalid EVM version requested.");
}
...
BOOST_AUTO_TEST_CASE(optimizer_settings_details_different)
{
	...
	BOOST_CHECK(
		getMemberNames(optimizer["details"]["yulDetails"]) ==
		(set<string>{"stackAllocation", "optimizerSteps"})
	);
	BOOST_CHECK(optimizer["details"]["yulDetails"]["stackAllocation"].get<bool>() == true);
	BOOST_CHECK(optimizer["details"]["yulDetails"]["optimizerSteps"].get<string>() == OptimiserSettings::DefaultYulOptimiserSteps);
	BOOST_CHECK_EQUAL(getMemberNames(optimizer["details"]).size(), 9);
	BOOST_CHECK(optimizer["runs"].get<Json::number_unsigned_t>() == 600);
}
...
BOOST_AUTO_TEST_CASE(license_in_metadata)
{
	...
	BOOST_CHECK(contract["metadata"].is_string());
	Json metadata;
	BOOST_REQUIRE(util::jsonParseStrict(contract["metadata"].get<string>(), metadata));
...
BOOST_AUTO_TEST_CASE(use_stack_optimization)
{
	...
	BOOST_CHECK(contract["evm"]["bytecode"]["object"].get<string>().length() > 20);
	...
	BOOST_CHECK(result["errors"][0]["message"].get<string>().find("Stack too deep when compiling inline assembly") != std::string::npos);
...
BOOST_AUTO_TEST_CASE(dependency_tracking_of_abstract_contract_yul)
{
	...
	const string& irCode = result["contracts"]["A.sol"]["C"]["ir"].get<string>();
...
BOOST_AUTO_TEST_CASE(source_location_of_bare_block)
{
	...
	string sourceMap = result["contracts"]["A.sol"]["A"]["evm"]["bytecode"]["sourceMap"].get<string>();
